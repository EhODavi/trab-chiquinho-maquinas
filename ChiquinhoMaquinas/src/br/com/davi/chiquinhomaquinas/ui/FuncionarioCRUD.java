...
public class FuncionarioCRUD extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        labelTitulo.setText("Gerenciamento de Funcionários");
    ...
    private void botaoExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoExcluirActionPerformed
        int opt = JOptionPane.showConfirmDialog(this, "Deseja realmente excluir?", "Exclusão de Funcionário", WIDTH);

        if (opt == JOptionPane.OK_OPTION) {
            try {
                Funcionario f = getFuncionarioFromTable();
                FuncionarioControle fControle = new FuncionarioControle();
                fControle.excluir(f.getCDPESSOA());
                ...
                JOptionPane.showMessageDialog(this, "Erro na Exclusão de Funcionário: " + e.getMessage());
            }
        }
    ...
    private void botaoAlterarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoAlterarActionPerformed
        Funcionario f = getFuncionarioFromTable();
        FuncionarioAlterarJDialog fAlt = new FuncionarioAlterarJDialog(this, true, f.getCDPESSOA());
        fAlt.setVisible(true);

        f = fAlt.getFuncionario();
        if (f != null) {
            int linha = tabela.getSelectedRow();
            tabela.setValueAt(f.getNOME(), linha, 1);
            tabela.setValueAt(f.getENDERECO(), linha, 2);
            if (f.getTELEFONE().length() == 8) {
                tabela.setValueAt("(31)" + f.getTELEFONE().substring(0, 4) + "-" + f.getTELEFONE().substring(4, 8), linha, 3);
            } else {
                tabela.setValueAt("(31)" + f.getTELEFONE().substring(0, 5) + "-" + f.getTELEFONE().substring(5, 9), linha, 3);
            }
            tabela.setValueAt(f.getEMAIL(), linha, 4);
            tabela.setValueAt(f.getCPF().substring(0, 3) + "." + f.getCPF().substring(3, 6) + "." + f.getCPF().substring(6, 9) + "-" + f.getCPF().substring(9, 11), linha, 5);
            tabela.setValueAt("R$" + Double.toString(f.getVALORHORA()), linha, 6);
        }
        desabilitarBotoes();
    ...
    private void botaoVisualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoVisualizarActionPerformed
        Funcionario f = getFuncionarioFromTable();
        FuncionarioVisualizarJDialog fVis = new FuncionarioVisualizarJDialog(this, true, f.getCDPESSOA());
    ...
    public Funcionario getFuncionarioFromTable() {
        ...
        if (linha >= 0) {
            ...
            try {
                fRet = fControle.consultarPorId(id);
            } catch (DaoException e) {
                JOptionPane.showMessageDialog(this, e.getMessage());
            }
        }
    ...
    private void consultarPorNome() {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Erro ao consultar Funcionário por Nome: " + e.getMessage());
        }
    }
